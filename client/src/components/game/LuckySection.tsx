...
export default function LuckySection({ 
  ...
  useEffect(() => {
    if (currentPhase === "betting" && gameSeconds >= 3 && gameSeconds <= 14) {
      ...
      if (shouldBet) {
        const availablePlayers = displayPlayers.filter(p => p.amount > 0 && !playerBets.has(p.id));
  ...
  }, [currentPhase, gameSeconds, displayPlayers, onPlaceBet, playerBets, roundWinner]);
